...
bucket_name = "sports-analytics-data-lake-proj-001"  # Change to a unique S3 bucket name
...
nfl_endpoint = os.getenv("NFL_ENDPOINT")  # Get NFL endpoint from .env
    ...
    try:
        glue_client.create_database(
            DatabaseInput={
                ...
                "Description": "Glue database for NFL sports analytics.",
            }
        )
...
def fetch_nfl_data():
    """Fetch NFL player data from sportsdata.io."""
    try:
        headers = {"Ocp-Apim-Subscription-Key": api_key}
        response = requests.get(nfl_endpoint, headers=headers)
        response.raise_for_status()  # Raise an error for bad status codes
        print("Fetched NFL data successfully.")
...
def create_glue_table():
    ...
    try:
        ...
        print(f"Glue table 'nfl_players' created successfully.")
    except Exception as e:
...
# Main workflow
def main():
    print("Setting up data lake for NFL sports analytics...")
    create_s3_bucket()
    time.sleep(5)  # Ensure bucket creation propagates
    create_glue_database()
    nfl_data = fetch_nfl_data()
    if nfl_data:  # Only proceed if data was fetched successfully
